...
console.log('[BRIEFING.JS] VERSION 2026-02-17-v2 loaded at', new Date().toISOString());
...
async function loadAgentStatus() {
    try {
        ...
        const data = await response.json();
        // API may return agents as dict keyed by id or as array
        let agentsList = data.agents || [];
        if (agentsList && !Array.isArray(agentsList)) {
            agentsList = Object.entries(agentsList).map(([id, info]) => ({ id, ...info }));
        }
        renderAgentConstellation(agentsList, data.fetched_at);
...
async function loadCrmActivity() {
    const container = document.getElementById('crm-activity');
    if (!container) return;
    
    try {
        const resp = await fetch('/api/crm/summary');
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const data = await resp.json();
        
        let html = '<div class="crm-summary">';
        
        // Counts row
        const c = data.counts || {};
        html += '<div class="crm-counts">';
        html += `<span class="crm-count-item"><strong>${c.Account || 0}</strong> Accounts</span>`;
        html += `<span class="crm-count-item"><strong>${c.Contact || 0}</strong> Contacts</span>`;
        html += `<span class="crm-count-item"><strong>${c.Opportunity || 0}</strong> Opportunities</span>`;
        html += `<span class="crm-count-item"><strong>${c.Lead || 0}</strong> Leads</span>`;
        html += '</div>';
        
        // Open opportunities
        if (data.open_opportunities && data.open_opportunities.length > 0) {
            html += '<div class="crm-section"><div class="crm-section-title">Open Opportunities</div>';
            data.open_opportunities.forEach(o => {
                const amount = o.amount ? `$${Number(o.amount).toLocaleString()}` : '';
                html += `<div class="crm-row">
                    <span class="crm-name">${escapeHtml(o.name || 'Untitled')}</span>
                    <span class="crm-stage">${escapeHtml(o.stage || '')}</span>
                    <span class="crm-amount">${amount}</span>
                </div>`;
            });
            html += '</div>';
        }
        ...
        if (data.upcoming_tasks && data.upcoming_tasks.length > 0) {
            html += '<div class="crm-section"><div class="crm-section-title">CRM Tasks</div>';
            data.upcoming_tasks.forEach(t => {
                const statusClass = t.status === 'Completed' ? 'done' : (t.priority === 'Urgent' ? 'urgent' : '');
                html += `<div class="crm-row ${statusClass}">
                    <span class="crm-name">${escapeHtml(t.name || 'Untitled')}</span>
                    <span class="crm-date">${t.dateEnd ? new Date(t.dateEnd).toLocaleDateString() : ''}</span>
                    <span class="crm-status">${escapeHtml(t.status || '')}</span>
                </div>`;
            });
            html += '</div>';
        }
        
        if (!data.open_opportunities?.length && !data.upcoming_tasks?.length) {
            html += '<div class="crm-empty">No active CRM items</div>';
        }
        ...
        container.innerHTML = '<div class="crm-empty">Unable to load CRM data</div>';
    }
}

// Refresh CRM every 10 minutes
setInterval(loadCrmActivity, 10 * 60 * 1000);
window.loadCrmActivity = loadCrmActivity;
